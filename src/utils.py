...
def get_mineral_cs_tuple() -> pd.DataFrame:
    ...
    mineral_attr['Crystal Systems'] = mineral_attr['Crystal Systems'].str.replace(r', +', '|')
    ...
    mineral_attr.rename(columns={'Mineral Name': 'mineral_name', 'Crystal Systems': 'crystal_system'}, inplace=True)
...
def get_mineral_pm_tuple() -> pd.DataFrame:
    ...
    mineral_pm['p47'] = mineral_pm[['p47a', 'p47b', 'p47c', 'p47d', 'p47e', 'p47f', 'p47g', 'p47h', 'p47i']].any(axis=1)
    mineral_pm[['p45', 'p47']] = mineral_pm[['p45', 'p47']].replace(False, np.nan)
    mineral_pm[['p45', 'p47']] = mineral_pm[['p45', 'p47']].replace(True, 1.0)
    mineral_pm = mineral_pm[mineral_pm.columns[mineral_pm.columns.str.match(r'p[0-9]+$')]]

    mineral_pm_tuple = mineral_pm.stack()
    mineral_pm_tuple = mineral_pm_tuple.reset_index().rename(columns={'level_1': 'paragenetic_mode'})
